<template>
  <ContainerComponent>
    <div class="flex justify-between items-center w-full">
      <div>
        <div class="text-6xl tracking-wide font-extrabold">
          {{ props.profile.name }}
        </div>
        <div class="mt-3 tracking-wide text-gray-700">
          <div>{{ props.profile.address }}</div>
          <div><a :href="'mailto:'+props.profile.email">{{ props.profile.email }}</a></div>
          <div><a :href="'https://wa.me/'+props.profile.phone">{{ props.profile.phone }}</a></div>
        </div>
      </div>
      <img
        :src="props.profile.photo"
        class="w-44 ring-1 ring-gray-300 ring-offset-1 mr-2"
      >
    </div>
    <div>
      <SectionTitle>Work Experiences</SectionTitle>
      <WorkExperiences :items="props.workExperiences" />
    </div>
    <div
      v-for="(section, index) in props.sections"
      :key="index"
    >
      <SectionTitle>{{ section.name }}</SectionTitle>
      <ExperienceItems :items="section.data" />
    </div>
  </ContainerComponent>
</template>

<script setup>
import ContainerComponent from '../components/ContainerComponent.vue'
import SectionTitle from '../components/SectionTitle.vue';
import WorkExperiences from '../components/WorkExperiences.vue';
import ExperienceItems from '../components/ExperienceItems.vue';

const props = defineProps({
  profile: Object,
  workExperiences: Array,
  sections: Array,
})
</script>
